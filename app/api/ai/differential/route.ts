...
//
// Doctor-only differential-diagnosis suggester.
...
export async function POST(req: NextRequest) {
  const blocked = await enforceRateLimit(req, "ai-differential", 30, "10 m");
  ...
  try {
    ...
    log.error("ai_differential.failed", err);
    ...
    return NextResponse.json(
      {
        error: /GEMINI_API_KEY/.test(msg)
          ...
          : "Could not produce differential. Try again.",
      },
      { status: 502 }
    );
  }
