  ...
  try {
    ...
    return NextResponse.json({
      allergies: listAllergies({
        ...
        type: (searchParams.get("type") as AllergyType) || undefined,
        severity: (searchParams.get("severity") as AllergySeverity) || undefined,
        verificationStatus:
...
export async function POST(req: NextRequest) {
  try {
    ...
    audit(ctx, { action: "create", entityType: "allergies", module: "allergies" });
    const __parsed_1 = await parseJson(req, z.any());
    if (__parsed_1 instanceof NextResponse) return __parsed_1;
    const body: any = __parsed_1;
    if (!body.patientId || !body.substance) {
      return NextResponse.json({ error: "missing_fields" }, { status: 400 });
    }
    return NextResponse.json({ allergy: createAllergy(orgId, body) });
  } catch (e) {
    return handleError(e);
  }
}
...
export async function PATCH(req: NextRequest) {
  try {
    const { ctx, orgId } = await requireActiveBilling(req);
    audit(ctx, { action: "update", entityType: "allergies", module: "allergies" });
    ...
    const a = updateAllergy(String(body.id), orgId, body);
    if (!a) return NextResponse.json({ error: "not_found" }, { status: 404 });
    return NextResponse.json({ allergy: a });
  } catch (e) {
    return handleError(e);
  }
}
...
export async function DELETE(req: NextRequest) {
  try {
    const { ctx, orgId } = await requireActiveBilling(req);
    audit(ctx, { action: "delete", entityType: "allergies", module: "allergies" });
    ...
    if (!body.id) return NextResponse.json({ error: "missing_id" }, { status: 400 });
    return NextResponse.json({ ok: deleteAllergy(String(body.id), orgId) });
  } catch (e) {
    return handleError(e);
  }
}
