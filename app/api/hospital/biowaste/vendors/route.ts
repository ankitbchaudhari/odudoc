...
export async function GET() {
  try {
    ...
    return NextResponse.json({ vendors: listVendors(orgId) });
  } catch (e) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ vendor: createVendor(orgId, body) });
  } catch (e) {
    return handleError(e);
  }
}
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const v = updateVendor(String(body.id), orgId, body);
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (!body.id) return NextResponse.json({ error: "missing_id" }, { status: 400 });
    return NextResponse.json({ ok: deleteVendor(String(body.id), orgId) });
  } catch (e) {
    return handleError(e);
  }
}
