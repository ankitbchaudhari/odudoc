  ...
  try {
    ...
    return NextResponse.json({
      sets: listSets({
        ...
        status: (searchParams.get("status") as SetStatus) || undefined,
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ set: createSet(orgId, body) });
  } catch (e) {
    return handleError(e);
  }
}
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const s = updateSet(String(body.id), orgId, body);
    if (!s) return NextResponse.json({ error: "not_found" }, { status: 404 });
    return NextResponse.json({ set: s });
  } catch (e) {
    return handleError(e);
  }
}
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (!body.id) return NextResponse.json({ error: "missing_id" }, { status: 400 });
    return NextResponse.json({ ok: deleteSet(String(body.id), orgId) });
  } catch (e) {
    return handleError(e);
  }
}
