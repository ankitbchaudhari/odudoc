...
export async function GET() {
  try {
    ...
    return NextResponse.json({ machines: listMachines(orgId) });
  } catch (e) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ machine: createMachine(orgId, body) });
  } catch (e) {
    return handleError(e);
  }
}
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const m = updateMachine(String(body.id), orgId, body);
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (!body.id) return NextResponse.json({ error: "missing_id" }, { status: 400 });
    const ok = deleteMachine(String(body.id), orgId);
    if (!ok)
      return NextResponse.json(
        { error: "in_use_or_not_found" },
        { status: 400 }
      );
