...
import {
  ...
  createDelivery,
  deleteDelivery,
} from "@/lib/hospital/dietary-store";
...
export async function POST(req: NextRequest) {
  try {
    ...
    audit(ctx, { action: "create", entityType: "dietary", module: "dietary" });
    ...
    const d = createDelivery(orgId, body);
    if (!d) return NextResponse.json({ error: "order_not_found" }, { status: 404 });
...
export async function DELETE(req: NextRequest) {
  try {
    const { ctx, orgId } = await requireActiveBilling(req);
    audit(ctx, { action: "delete", entityType: "dietary", module: "dietary" });
