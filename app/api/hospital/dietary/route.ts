  ...
  try {
    ...
    return NextResponse.json({
      orders: listOrders({
        ...
        status: (searchParams.get("status") as DietStatus) || undefined,
        dietType: (searchParams.get("dietType") as DietType) || undefined,
      }),
    });
  } catch (e) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ order: createOrder(orgId, body) });
  } catch (e) {
    return handleError(e);
  }
}
...
export async function PATCH(req: NextRequest) {
  try {
    const { ctx, orgId } = await requireActiveBilling(req);
    audit(ctx, { action: "update", entityType: "dietary", module: "dietary" });
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (!body.id) return NextResponse.json({ error: "missing_id" }, { status: 400 });
    return NextResponse.json({ ok: deleteOrder(String(body.id), orgId) });
  } catch (e) {
    return handleError(e);
  }
}
