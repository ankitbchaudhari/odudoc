...
import { listExams, createExam, updateExam, deleteExam, computeStats, type EntStatus, type EntVisitType, type EntRegion } from "@/lib/hospital/ent-store";
  ...
  try {
    ...
    return NextResponse.json({
      exams: listExams({
        ...
        status: (searchParams.get("status") as EntStatus) || undefined,
        visitType: (searchParams.get("visitType") as EntVisitType) || undefined,
        region: (searchParams.get("region") as EntRegion) || undefined,
...
export async function POST(req: NextRequest) {
  try {
    ...
    audit(ctx, { action: "create", entityType: "ent", module: "ent" });
...
export async function PATCH(req: NextRequest) {
  try {
    const { ctx, orgId } = await requireActiveBilling(req);
    audit(ctx, { action: "update", entityType: "ent", module: "ent" });
...
export async function DELETE(req: NextRequest) {
  try {
    const { ctx, orgId } = await requireActiveBilling(req);
    audit(ctx, { action: "delete", entityType: "ent", module: "ent" });
