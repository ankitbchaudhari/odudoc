  ...
  try {
    ...
    return NextResponse.json({
      deliveries: listDeliveries({
        ...
        motherId: searchParams.get("motherId") || undefined,
...
export async function POST(req: NextRequest) {
  try {
    ...
    audit(ctx, { action: "create", entityType: "maternity", module: "maternity" });
    const __parsed_1 = await parseJson(req, z.any());
    if (__parsed_1 instanceof NextResponse) return __parsed_1;
    const body: any = __parsed_1;
    const res = createDelivery(orgId, body);
    if (!res.ok) return NextResponse.json({ error: res.error }, { status: 400 });
    return NextResponse.json({ delivery: res.delivery });
  } catch (e) {
    return handleError(e);
  }
}
...
export async function PATCH(req: NextRequest) {
  try {
    const { ctx, orgId } = await requireActiveBilling(req);
    audit(ctx, { action: "update", entityType: "maternity", module: "maternity" });
    ...
    const d = updateDelivery(String(body.id), orgId, body);
...
export async function DELETE(req: NextRequest) {
  try {
    const { ctx, orgId } = await requireActiveBilling(req);
    audit(ctx, { action: "delete", entityType: "maternity", module: "maternity" });
    ...
    if (!body.id) return NextResponse.json({ error: "missing_id" }, { status: 400 });
    return NextResponse.json({ ok: deleteDelivery(String(body.id), orgId) });
  } catch (e) {
    return handleError(e);
  }
}
