...
import {
  listAudits, getAudit, createAudit, updateAudit, deleteAudit, computeStats,
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (id) {
      const a = getAudit(id, orgId);
      if (!a) return NextResponse.json({ error: "not_found" }, { status: 404 });
      return NextResponse.json({ audit: a });
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    audit(ctx, { action: "create", entityType: "mortality-audit", module: "mortality-audit" });
    const __parsed_1 = await parseJson(req, z.any());
    if (__parsed_1 instanceof NextResponse) return __parsed_1;
    const body: any = __parsed_1;
    const r = createAudit(orgId, body);
    if (!r.ok) return NextResponse.json({ error: r.error }, { status: 400 });
    return NextResponse.json({ audit: r.record });
  } catch (e) { return handleError(e); }
}
...
export async function PATCH(req: NextRequest) {
  try {
    const { ctx, orgId } = await requireActiveBilling(req);
    audit(ctx, { action: "update", entityType: "mortality-audit", module: "mortality-audit" });
    ...
    const r = updateAudit(String(body.id), orgId, body);
    if (!r) return NextResponse.json({ error: "not_found" }, { status: 404 });
    return NextResponse.json({ audit: r });
  } catch (e) { return handleError(e); }
}
...
export async function DELETE(req: NextRequest) {
  try {
    const { ctx, orgId } = await requireActiveBilling(req);
    audit(ctx, { action: "delete", entityType: "mortality-audit", module: "mortality-audit" });
    ...
    if (!body.id) return NextResponse.json({ error: "missing_id" }, { status: 400 });
    return NextResponse.json({ ok: deleteAudit(String(body.id), orgId) });
  } catch (e) { return handleError(e); }
}
