  ...
  try {
    ...
    return NextResponse.json({
      cycles: listCycles({
        ...
        status: (searchParams.get("status") as CycleStatus) || undefined,
...
export async function POST(req: NextRequest) {
  try {
    ...
    const res = createCycle(orgId, body);
    if (!res.ok) return NextResponse.json({ error: res.error }, { status: 400 });
    return NextResponse.json({ cycle: res.cycle });
  } catch (e) {
    return handleError(e);
  }
}
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const c = updateCycle(String(body.id), orgId, body);
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (!body.id) return NextResponse.json({ error: "missing_id" }, { status: 400 });
    return NextResponse.json({ ok: deleteCycle(String(body.id), orgId) });
  } catch (e) {
    return handleError(e);
  }
}
