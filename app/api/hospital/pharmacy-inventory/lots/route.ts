...
import { listLots, createLot, updateLot, deleteLot } from "@/lib/hospital/pharmacy-inventory-store";
...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ lots: listLots({ organizationId: orgId, itemId: searchParams.get("itemId") || undefined }) });
...
export async function POST(req: NextRequest) {
  try {
    ...
    const res = createLot(orgId, body);
    if (!res.ok) return NextResponse.json({ error: res.error }, { status: 400 });
    return NextResponse.json({ lot: res.lot });
  } catch (e) { return handleError(e); }
}
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const l = updateLot(String(body.id), orgId, body);
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (!body.id) return NextResponse.json({ error: "missing_id" }, { status: 400 });
    return NextResponse.json({ ok: deleteLot(String(body.id), orgId) });
  } catch (e) { return handleError(e); }
}
