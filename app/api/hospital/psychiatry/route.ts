...
import {
  ...
  computeStats,
  type SessionStatus, type PlanStatus,
} from "@/lib/hospital/psychiatry-store";
...
export async function POST(req: NextRequest) {
  try {
    ...
    audit(ctx, { action: "create", entityType: "psychiatry", module: "psychiatry" });
...
export async function PATCH(req: NextRequest) {
  try {
    const { ctx, orgId } = await requireActiveBilling(req);
    audit(ctx, { action: "update", entityType: "psychiatry", module: "psychiatry" });
...
export async function DELETE(req: NextRequest) {
  try {
    const { ctx, orgId } = await requireActiveBilling(req);
    audit(ctx, { action: "delete", entityType: "psychiatry", module: "psychiatry" });
