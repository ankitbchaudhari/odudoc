...
export async function GET() {
  try {
    ...
    return NextResponse.json({ counters: listCounters(orgId) });
  } catch (e) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ counter: createCounter(orgId, body) });
  } catch (e) {
    return handleError(e);
  }
}
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const c = updateCounter(String(body.id), orgId, body);
    if (!c) return NextResponse.json({ error: "not_found" }, { status: 404 });
    return NextResponse.json({ counter: c });
  } catch (e) {
    return handleError(e);
  }
}
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (!body.id) return NextResponse.json({ error: "missing_id" }, { status: 400 });
    return NextResponse.json({ ok: deleteCounter(String(body.id), orgId) });
  } catch (e) {
    return handleError(e);
  }
}
