...
} from "@/lib/hospital/referrals-store";
  ...
  try {
    ...
    return NextResponse.json({
      referrals: listReferrals({
        ...
        direction:
          (searchParams.get("direction") as ReferralDirection) || undefined,
        status: (searchParams.get("status") as ReferralStatus) || undefined,
        urgency: (searchParams.get("urgency") as ReferralUrgency) || undefined,
      }),
    });
  } catch (e) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    audit(ctx, { action: "create", entityType: "referrals", module: "referrals" });
    ...
    return NextResponse.json({ referral: createReferral(orgId, body) });
  } catch (e) {
    return handleError(e);
  }
}
...
export async function PATCH(req: NextRequest) {
  try {
    const { ctx, orgId } = await requireActiveBilling(req);
    audit(ctx, { action: "update", entityType: "referrals", module: "referrals" });
    ...
    const r = updateReferral(String(body.id), orgId, body);
...
export async function DELETE(req: NextRequest) {
  try {
    const { ctx, orgId } = await requireActiveBilling(req);
    audit(ctx, { action: "delete", entityType: "referrals", module: "referrals" });
    ...
    if (!body.id) return NextResponse.json({ error: "missing_id" }, { status: 400 });
    return NextResponse.json({ ok: deleteReferral(String(body.id), orgId) });
  } catch (e) {
    return handleError(e);
  }
}
