...
import { listShifts, createShift, updateShift, deleteShift, type ShiftStatus, type Department } from "@/lib/hospital/staff-schedule-store";
  ...
  try {
    ...
    return NextResponse.json({
      shifts: listShifts({
        ...
        status: (searchParams.get("status") as ShiftStatus) || undefined,
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const s = updateShift(String(body.id), orgId, body);
    if (!s) return NextResponse.json({ error: "not_found" }, { status: 404 });
    return NextResponse.json({ shift: s });
  } catch (e) { return handleError(e); }
}
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (!body.id) return NextResponse.json({ error: "missing_id" }, { status: 400 });
    return NextResponse.json({ ok: deleteShift(String(body.id), orgId) });
  } catch (e) { return handleError(e); }
}
