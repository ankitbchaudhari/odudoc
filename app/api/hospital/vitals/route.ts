...
import {
  ...
  updateReading,
  deleteReading,
} from "@/lib/hospital/vitals-store";
...
export async function POST(req: NextRequest) {
  try {
    ...
    audit(ctx, { action: "create", entityType: "vitals", module: "vitals" });
    ...
    return NextResponse.json({ reading: createReading(orgId, body) });
  } catch (e) {
    return handleError(e);
  }
}
...
export async function PATCH(req: NextRequest) {
  try {
    const { ctx, orgId } = await requireActiveBilling(req);
    audit(ctx, { action: "update", entityType: "vitals", module: "vitals" });
    ...
    const r = updateReading(String(body.id), orgId, body);
...
export async function DELETE(req: NextRequest) {
  try {
    const { ctx, orgId } = await requireActiveBilling(req);
    audit(ctx, { action: "delete", entityType: "vitals", module: "vitals" });
    ...
    if (!body.id) return NextResponse.json({ error: "missing_id" }, { status: 400 });
    return NextResponse.json({ ok: deleteReading(String(body.id), orgId) });
  } catch (e) {
    return handleError(e);
  }
}
