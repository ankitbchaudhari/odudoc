...
const PLATFORM_CAPABILITIES = [
  ...
  { icon: "💬", title: "WhatsApp customer-care chatbot", desc: "Inbound WhatsApp messages route to a stateless intent router (STOP / HELP / MENU / BOOK / BALANCE / SUPPORT) over Meta Cloud API. Freeform replies inside the 24h customer-service window cost ₹0 — the bot effectively self-funds." },
  { icon: "📨", title: "16 approved WhatsApp templates", desc: "OTP, appointment confirm + reminder + cancellation, lab ready, Rx delivered, doctor alert on new booking, welcome, payment failed, refund processed, wallet top-up, withdrawal processed, vital alert, insurance pre-auth, care-plan reminder, follow-up nudge, doctor invite — all Meta-approved, fired via sent.dm BSP." },
  { icon: "📊", title: "WhatsApp delivery dashboard", desc: "Admin sees every WA send live — masked recipient, template name, success/failure, error reason, provider message id. Today's send count, success rate, top template by volume, top failure reason. Filter by template or status." },
  { icon: "✨", title: "AI care-plan generator", desc: "Patient picks a chronic condition (diabetes / hypertension / asthma…); AI drafts title, 4–7 measurable lifestyle goals, and a personalised note. Multi-provider fallback chain (Gemini → OpenAI → Anthropic). Patient reviews and edits before saving." },
  { icon: "⌚", title: "Wearables → vitals merge", desc: "Linked Fitbit / Apple Health / Google Health readings (HR, SpO₂, BP, glucose, weight, temperature, respiration) flow into the patient's My Vitals timeline as one stream. BP systolic + diastolic paired by timestamp. Source badge on each row; deletes route to wearable management." },
  { icon: "🛡️", title: "Country-aware insurance directory", desc: "60+ pre-loaded insurers across India (Star, HDFC ERGO, Niva Bupa, MediAssist), US (UnitedHealthcare, Aetna, Cigna, Medicare), UK (Bupa, AXA, NHS), UAE (Daman, Thiqa), and 6 more countries. Patient's country drives the dropdown; sum-insured label adapts to local currency symbol." },
  { icon: "💊", title: "EMR + consult-aware Rx", desc: "Doctor opens an EMR patient record or a live video consult → taps Write / AI / Voice Prescription → form arrives with patient name, age, gender, phone, ID, chief complaint all auto-filled. No re-typing. Single prefill hook serves all three Rx generators." },
  { icon: "💵", title: "Zero-loss payment buffer", desc: "Cashfree webhooks that arrive before the booking row exists are parked in a pending-payments buffer keyed by orderId, then reconciled when the booking finishes verifying. Integrity-check cron flags any row unreconciled past 48h for ops review." },
  { icon: "📅", title: "Daily nudge crons", desc: "Three Vercel crons keep patients engaged without spam: appointment reminder 24h before (08:00 UTC), care-plan check-in for chronic patients (07:00 UTC), follow-up nudge ~14 days post-consult (10:00 UTC). All idempotent, all bypass opted-out phones." },
  { icon: "🪪", title: "Distinct patient + doctor identifiers", desc: "Patients carry NNN-NNNNN-NNNNN-NNN; doctors carry DR-NNN-NNNNN-NNNNN-NNN. Same 3-5-5-3 body so they feel like one family, but DR-prefix makes the role obvious on visiting cards, support tickets, and database rows." },
  { icon: "🧾", title: "Branded prescription templates", desc: "23 prescription layouts — Classic Blue, Modern Minimal, Teal Gradient, Corporate Navy, Emerald Fresh, Rose Elegance, and 17 more. Every layout renders the OduDoc gradient logo in the header and a faded watermark behind the content. PDF + share + email all carry it." },
  { icon: "🌍", title: "Global form catalogue", desc: "192-country dial-code picker with flag emojis, 80-zone IANA time zone selector, 10 ISO-tagged insurance regions. Reusable across signup, admin org form, enterprise quote form, and patient profile." },
];
