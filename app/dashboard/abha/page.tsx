  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-6">
      ...
      <div className="mb-6 flex items-start justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">ABHA / ABDM</h1>
      ...
      <section className="mb-6 rounded-2xl bg-white p-5 shadow-sm">
        <p className="mb-3 text-sm font-bold text-slate-900">Linked accounts ({links.length})</p>
        {links.length === 0 ? (
          <p className="rounded-lg bg-slate-50 p-4 text-sm text-slate-500">No ABHA linked yet. Press &ldquo;+ Link ABHA&rdquo; — we&apos;ll send a one-time code to your mobile.</p>
        ) : (
          <ul className="space-y-2">
            {links.map((l) => (
              <li key={l.id} className="flex items-start justify-between gap-3 rounded-lg border border-slate-200 p-3">
                <div>
                  <p className="font-mono text-sm font-bold text-slate-900">{l.abhaNumber}</p>
                  <p className="text-xs text-slate-500">{l.abhaAddress}{l.kycSource ? ` · KYC: ${l.kycSource}` : ""}</p>
      ...
      <section className="rounded-2xl bg-white p-5 shadow-sm">
        <div className="mb-3 flex items-center justify-between">
          <p className="text-sm font-bold text-slate-900">Discoverable records ({contexts.length})</p>
          <span className="text-[10px] text-slate-500">Care contexts other PHR apps can request via consent</span>
        </div>
        {contexts.length === 0 ? (
          <p className="rounded-lg bg-slate-50 p-4 text-sm text-slate-500">No care contexts yet. Once you have an encounter / Rx / lab report at any OduDoc-connected hospital, it appears here as a discoverable record.</p>
        ) : (
          <ul className="space-y-2">
            {contexts.map((c) => (
              <li key={c.id} className="rounded-lg border border-slate-200 p-3">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="font-semibold text-slate-900">{c.display}</p>
                    <p className="text-[11px] text-slate-500">{c.type} · {new Date(c.recordDate).toLocaleDateString()}{c.nhaContextId ? ` · NHA ${c.nhaContextId.slice(0, 14)}…` : ""}</p>
                  </div>
      ...
      {showStart && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => { setShowStart(false); reset(); }}>
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-bold text-slate-900">Link your ABHA</h3>
            ...
            {phase === "start" ? (
              <div className="mt-4 space-y-3">
                ...
                <p className="text-[10px] text-slate-500">e.g. ankit.chaudhari@abdm — leave blank to auto-generate.</p>
                <div className="flex justify-end gap-2">
                  <button onClick={() => { setShowStart(false); reset(); }} className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700">Cancel</button>
              ...
              <div className="mt-4 space-y-3">
                ...
                <div className="flex justify-end gap-2">
                  <button onClick={() => setPhase("start")} className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700">Back</button>
