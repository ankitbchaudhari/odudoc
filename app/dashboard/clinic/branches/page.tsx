    ...
    <div>
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Manage locations across cities.</p>
        </div>
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
        <Card label="Total branches" value={items.length} color="text-gray-900" />
        <Card label="Active" value={items.filter((b) => b.active).length} color="text-emerald-700" />
        <Card label="Inactive" value={items.filter((b) => !b.active).length} color="text-gray-500" />
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {items.length === 0 ? (
          <p className="col-span-full rounded-xl bg-white p-10 text-center text-sm text-gray-400">No branches yet.</p>
        ) : (
          items.map((b) => (
            <div key={b.id} className="rounded-xl bg-white p-4 shadow-sm">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <p className="mt-1 text-xs text-gray-500">{b.address || "—"}</p>
                  <p className="mt-2 text-xs text-gray-500">
                    Manager: <span className="text-gray-700">{b.manager || "—"}</span>
                  </p>
