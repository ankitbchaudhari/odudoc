  ...
  return (
    <div>
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Patients (EHR)</h1>
          <p className="mt-1 text-sm text-gray-500">Unlimited electronic health records — HIPAA &amp; GDPR compliant.</p>
        </div>
        <button
          onClick={() => {
            setForm(blank);
      ...
      <div className="mb-4">
        <input
          ...
          className="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20"
        />
      </div>

      {showForm && (
        <form onSubmit={handleSave} className="mb-4 grid grid-cols-1 gap-3 rounded-xl bg-white p-4 shadow-sm sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            <label className="block">
              <span className="text-xs font-medium text-gray-600">Clinical notes</span>
              <textarea
                value={form.notes}
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {filtered.length === 0 ? (
          <p className="col-span-full rounded-xl bg-white p-10 text-center text-sm text-gray-400">
            ...
            <div key={p.id} className="rounded-xl bg-white p-4 shadow-sm">
              <div className="flex items-start justify-between">
                ...
                <div className="flex gap-1">
                  <button onClick={() => startEdit(p)} className="rounded p-1.5 text-gray-400 hover:bg-gray-100 hover:text-primary-600" aria-label="Edit">
                    ✎
                  </button>
                  <button onClick={() => remove(p.id)} className="rounded p-1.5 text-gray-400 hover:bg-red-50 hover:text-red-600" aria-label="Delete">
                    🗑
                  </button>
                </div>
              </div>
              {p.conditions && (
                <p className="mt-2 text-xs">
                  ...
                  <span className="text-gray-700">{p.conditions}</span>
                </p>
              )}
              {p.allergies && (
              ...
              {p.notes && (
                <p className="mt-2 rounded bg-gray-50 p-2 text-xs text-gray-700">{p.notes}</p>
              )}
              <p className="mt-2 text-xs text-gray-400">📞 {p.phone || "—"} · {p.email || "—"}</p>
            </div>
          ))
        )}
      </div>
