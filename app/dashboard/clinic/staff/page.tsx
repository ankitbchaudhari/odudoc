    ...
    <div>
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Manage staff, roles, and monthly payroll.</p>
        </div>
        <button
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
        <Card label="Team size" value={items.length} color="text-gray-900" />
      ...
      <div className="overflow-hidden rounded-xl bg-white shadow-sm">
        {items.length === 0 ? (
          <p className="p-10 text-center text-sm text-gray-400">No staff yet.</p>
        ) : (
            ...
            <tbody className="divide-y divide-gray-100">
              {items.map((s) => (
                <tr key={s.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-600">{s.joinedAt}</td>
                  <td className="px-4 py-3 font-semibold text-gray-900">${s.salary.toLocaleString()}</td>
