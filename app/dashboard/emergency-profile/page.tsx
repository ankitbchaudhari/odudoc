...
export default function EmergencyProfilePage() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Emergency profile</h1>
      ...
      <section className="rounded-2xl bg-white dark:bg-slate-900 p-5 shadow-sm ring-1 ring-slate-200 dark:ring-slate-800">
        <p className="mb-3 text-sm font-bold text-slate-900 dark:text-slate-100">Critical facts</p>
      ...
      <section className="mt-6 rounded-2xl bg-white dark:bg-slate-900 p-5 shadow-sm ring-1 ring-slate-200 dark:ring-slate-800">
        <p className="mb-3 text-sm font-bold text-slate-900 dark:text-slate-100">Care contacts</p>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
      ...
      <section className="mt-6 rounded-2xl bg-white dark:bg-slate-900 p-5 shadow-sm ring-1 ring-slate-200 dark:ring-slate-800">
        <p className="mb-3 text-sm font-bold text-slate-900 dark:text-slate-100">Notes</p>
      ...
      <section className="mt-6 rounded-2xl bg-white dark:bg-slate-900 p-5 shadow-sm ring-1 ring-slate-200 dark:ring-slate-800">
        <div className="mb-3 flex items-center justify-between">
          <p className="text-sm font-bold text-slate-900 dark:text-slate-100">Biometric enrollments</p>
        ...
        {enrollments.length === 0 ? (
          ...
          <ul className="space-y-2">
            {enrollments.map((e) => (
              <li key={e.id} className="flex items-center justify-between rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 px-3 py-2 text-xs">
                <div>
                  <span className="font-bold text-slate-900 dark:text-slate-100">{e.kind === "fingerprint" ? "🔒 Fingerprint" : "👤 Face"}</span>
                  <span className="ml-2 text-slate-500 dark:text-slate-400">{new Date(e.enrolledAt).toLocaleDateString()}{!e.active && " · paused"}</span>
                </div>
                {e.active && (
      ...
      <div className="sticky bottom-0 mt-6 -mx-4 border-t border-slate-200 dark:border-slate-800 bg-white/95 px-4 py-3 backdrop-blur-md">
...
function Field({ label, value, onChange, placeholder, textarea }: { label: string; value?: string; onChange: (v: string) => void; placeholder?: string; textarea?: boolean }) {
  return (
    <label className={`text-xs font-semibold text-slate-700 dark:text-slate-300 ${textarea ? "sm:col-span-2" : ""}`}>
      {label}
      {textarea ? (
        <textarea value={value || ""} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} rows={3} className="mt-1 w-full rounded-lg border border-slate-300 bg-white dark:bg-slate-900 px-3 py-2 text-sm font-normal" />
      ) : (
