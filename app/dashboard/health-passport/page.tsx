...
export default function HealthPassportPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-6">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-900">Health Passport</h1>
      ...
      <div className="grid gap-6 lg:grid-cols-[auto_1fr]">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm text-center">
          {loading || !qr ? (
            ...
            <>
              <p className="mb-1 text-[10px] font-bold uppercase tracking-wider text-slate-500">Health Passport for</p>
              <p className="text-lg font-bold text-slate-900">{qr.profile.name}</p>
              ...
              <div className="my-4 inline-block rounded-xl bg-white p-3 ring-2 ring-indigo-200">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={qr.qrUrl} alt="Health Passport QR" width={300} height={300} className="block" />
              </div>
              <p className="font-mono text-xs text-slate-500">{qr.profile.formattedMedicalId}</p>
              <p className="mt-3 max-w-[300px] text-xs text-slate-500">
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 className="text-lg font-bold text-slate-900">Consent vault</h2>
                  ...
                  <li key={c.id} className="rounded-lg border border-emerald-200 bg-emerald-50/50 p-3">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1">
                        <p className="font-semibold text-slate-900">{c.orgName}</p>
                        ...
                        {c.note && <p className="mt-1 text-[11px] italic text-slate-500">&ldquo;{c.note}&rdquo;</p>}
                      </div>
          ...
          {inactive.length > 0 && (
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <p className="text-xs font-bold uppercase tracking-wider text-slate-500">Past grants ({inactive.length})</p>
              <ul className="mt-3 space-y-1">
                {inactive.slice(0, 8).map((c) => (
                  <li key={c.id} className="flex items-center justify-between gap-2 rounded-md bg-slate-50 px-3 py-2 text-xs">
