...
export default function InsurancePage() {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-6">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-900">Insurance & Cashless</h1>
      ...
      <section className="mb-8 rounded-2xl bg-white p-5 shadow-sm">
        ...
        {policies.length === 0 ? (
          <p className="rounded-lg bg-slate-50 p-4 text-sm text-slate-500">No policies linked yet. Add your insurance card details so hospitals can run cashless on your behalf.</p>
        ) : (
          <ul className="space-y-2">
            {policies.map((p) => (
              <li key={p.id} className={`rounded-xl border p-4 ${p.isPrimary ? "border-indigo-300 bg-indigo-50/40" : "border-slate-200 bg-white"}`}>
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="font-mono text-xs text-slate-500">Member ID {p.memberId}</p>
                    <div className="mt-1 flex flex-wrap gap-2 text-xs text-slate-500">
                      {p.planName && <span className="rounded-md bg-slate-100 px-2 py-0.5">{p.planName}</span>}
                  ...
                  <div className="flex flex-col gap-1">
                    {!p.isPrimary && <button onClick={() => setPrimary(p.id)} className="rounded-md border border-slate-300 px-2 py-1 text-[11px] font-semibold text-slate-700">Set primary</button>}
      ...
      {/* ── Coverage estimator ────────────────────────────── */}
      <section className="mb-8 rounded-2xl bg-white p-5 shadow-sm">
        <h2 className="mb-3 text-lg font-bold text-slate-900">Coverage estimator</h2>
        <p className="mb-3 text-xs text-slate-500">Get a sticker price before any procedure. Final numbers come from the TPA after pre-auth — this is the upfront estimate.</p>
        ...
        {est && (
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            <div className="rounded-xl bg-slate-50 p-4">
              <p className="text-[11px] uppercase tracking-wider text-slate-500">Total bill (gross)</p>
              <p className="mt-1 text-2xl font-extrabold text-slate-900">{fmtINR(est.grossRupees)}</p>
            ...
            <div className="rounded-xl bg-rose-50 p-4">
              ...
              <p className="text-[10px] text-slate-500">Confidence: {Math.round(est.confidence * 100)}%</p>
            </div>
            ...
            <div className="sm:col-span-3 rounded-xl bg-white p-4 ring-1 ring-slate-200">
              <p className="mb-2 text-xs font-bold uppercase tracking-wider text-slate-500">Breakdown</p>
              <ul className="space-y-1 text-sm">
                {est.breakdown.map((b, i) => (
                  <li key={i} className="flex justify-between border-b border-slate-100 py-1">
                    <span className="text-slate-700">{b.label}</span>
                    <span className={`font-mono font-semibold ${b.amount < 0 ? "text-emerald-700" : "text-slate-900"}`}>{b.amount < 0 ? "−" : ""}{fmtINR(Math.abs(b.amount))}</span>
                  </li>
                ))}
              </ul>
            </div>
      ...
      <section className="rounded-2xl bg-white p-5 shadow-sm">
        <h2 className="mb-3 text-lg font-bold text-slate-900">My pre-authorizations</h2>
        {preauths.length === 0 ? (
          <p className="rounded-lg bg-slate-50 p-4 text-sm text-slate-500">No pre-auths yet. When a hospital files one for you, it shows up here.</p>
        ) : (
          <ul className="space-y-2">
            {preauths.map((p) => (
              <li key={p.id} className="rounded-xl border border-slate-200 bg-white p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="font-semibold text-slate-900">{p.procedureName}</p>
      ...
      {showAdd && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setShowAdd(false)}>
          <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-bold text-slate-900">Add insurance policy</h3>
            ...
            <div className="mt-5 flex justify-end gap-2">
              <button onClick={() => setShowAdd(false)} className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700">Cancel</button>
