...
// In-person clinic appointment booking. URL:
//   /doctors/:doctorId/book-clinic/:clinicId
//
// Simpler than the telemedicine BookingModal:
//   1) Pick date + slot (uses doctor.timeSlots)
//   2) Patient details (name, phone, email)
//   3) Payment mode: online OR at clinic
//   4) (Online only) → Stripe/Cashfree handled by the existing booking
//      payment flow. For v1 we go straight to "pay at clinic" path on
//      the simple page; online-payment users can still use the
//      telemedicine BookingModal which has full gateway support.
//   5) POST /api/bookings with clinicId + paymentMode
//   6) Redirect to /booking/:id (confirmation w/ QR)
...
export default function BookClinicPage() {
  const params = useParams<{ id: string; clinicId: string }>();
  ...
  const [paymentMode, setPaymentMode] = useState<"online" | "clinic">("clinic");

  const [err, setErr] = useState<string | null>(null);
  const [busy, setBusy] = useState(false);

  useEffect(() => {
    if (!params.id || !params.clinicId) return;
    setLoading(true);
    Promise.all([
      fetch(`/api/doctors/${params.id}`).then((r) => r.json()).catch(() => ({})),
      fetch(`/api/clinics/by-doctor/${params.id}`).then((r) => r.json()).catch(() => ({})),
    ]).then(([dRes, cRes]) => {
      const d = dRes.doctor;
      if (d) setDoctor({ id: d.id, name: d.name, fee: d.fee, timeSlots: d.timeSlots || [], email: d.email, phone: d.phone });
      const cl = (cRes.clinics || []).find((c: Clinic) => c.id === params.clinicId);
      if (cl) {
        setClinic(cl);
        // Default payment mode: prefer clinic-pay if both enabled (simpler
        // flow); only auto-select 'online' when at-clinic isn't allowed.
        if (!cl.acceptClinicPayment && cl.acceptOnlinePayment) setPaymentMode("online");
        if (!cl.acceptOnlinePayment) setPaymentMode("clinic");
      }
    }).finally(() => setLoading(false));
  }, [params.id, params.clinicId]);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (paymentMode === "online") {
      // The full online-payment flow uses the BookingModal (Stripe / Cashfree).
      // For v1 we redirect users to the doctor profile and ask them to use
      // the "Book Appointment" button there, passing the clinic intent
      // through the URL hash so the modal can pre-select it once that
      // integration lands.
  ...
  if (loading) return <main className="p-8 text-sm text-gray-500 dark:text-slate-400">Loading…</main>;
  if (!doctor || !clinic) {
    return (
      <main className="mx-auto max-w-xl px-4 py-10">
        <p className="text-sm text-red-700">Clinic or doctor not found.</p>
        <Link href="/doctors" className="mt-3 inline-block text-sm text-primary-600 hover:underline">← All doctors</Link>
      </main>
    );
  }
