...
export default function ProductDetailPage() {
  ...
  if (loading) {
    return <div className="p-12 text-center text-sm text-gray-400 dark:text-slate-500">Loading product…</div>;
  }
    ...
    return (
      <div className="flex min-h-[60vh] items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-500 dark:text-slate-400">The product you are looking for does not exist.</p>
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900">
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <nav className="mb-6 flex items-center gap-2 text-sm text-gray-500 dark:text-slate-400">
          ...
          <span className="font-medium text-gray-900 dark:text-slate-100">{product.name}</span>
        </nav>
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          ...
          <div>
            ...
            <h1 className="mt-3 text-2xl font-bold text-gray-900 dark:text-slate-100 md:text-3xl">{product.name}</h1>
            {product.vendorName && (
              <p className="mt-1 text-sm text-gray-500 dark:text-slate-400">
                ...
                ) : <span className="font-medium text-gray-700 dark:text-slate-300">{product.vendorName}</span>}
              </p>
            )}
            ...
            <div className="mt-3 flex items-center gap-2">
              ...
              <span className="text-sm text-gray-500 dark:text-slate-400">{product.rating} ({product.reviewCount} reviews)</span>
            </div>

            {/* Price */}
            <div className="mt-4 flex items-baseline gap-3">
              <span className="text-3xl font-bold text-gray-900 dark:text-slate-100">${product.price.toFixed(2)}</span>
              {product.originalPrice && (
                <>
                  <span className="text-xl text-gray-400 dark:text-slate-500 line-through">${product.originalPrice.toFixed(2)}</span>
            ...
            {/* Description */}
            <p className="mt-4 text-gray-600 dark:text-slate-300 leading-relaxed">{product.description}</p>

            {/* Benefits */}
            <div className="mt-5">
              <h3 className="text-sm font-semibold text-gray-900 dark:text-slate-100">Key Benefits</h3>
        ...
        <div className="mt-12">
          ...
          <div className="mt-6 rounded-xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6">
            {activeTab === "description" && (
              <p className="text-gray-600 dark:text-slate-300 leading-relaxed">{product.fullDescription}</p>
            )}
            ...
            {activeTab === "reviews" && (
              <div>
                ...
                <div className="mb-8 grid gap-6 md:grid-cols-2">
                  <div>
                    <div className="text-center">
                      <span className="text-5xl font-bold text-gray-900 dark:text-slate-100">{product.rating}</span>
                      ...
                      <p className="mt-1 text-sm text-gray-500 dark:text-slate-400">{product.reviewCount} reviews</p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    {ratingBreakdown.map((rb) => (
                      <div key={rb.star} className="flex items-center gap-3">
                        <span className="w-8 text-right text-sm text-gray-600 dark:text-slate-300">{rb.star}</span>
                        ...
                        <span className="w-8 text-sm text-gray-500 dark:text-slate-400">{rb.count}</span>
                      </div>
                    ))}
                  </div>
                </div>
                    ...
                    <div key={i} className="border-t border-gray-100 pt-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm font-medium text-gray-900 dark:text-slate-100">{review.name}</p>
                            <p className="text-xs text-gray-400 dark:text-slate-500">{review.date}</p>
                          </div>
                        </div>
                        <div className="flex">
                          {[...Array(5)].map((_, j) => (
                      ...
                      <p className="mt-2 text-sm text-gray-600 dark:text-slate-300">{review.text}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="mt-6 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
              {relatedProducts.map((rp) => (
                <Link
                  key={rp.id}
                  href={`/shop/${rp.id}`}
                  className="group overflow-hidden rounded-xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg"
                >
                  ...
                  <div className="p-4">
                    <h3 className="font-semibold text-gray-900 dark:text-slate-100 group-hover:text-primary-600">
                      {rp.name}
                    </h3>
                    <div className="mt-2 flex items-baseline gap-2">
                      <span className="text-lg font-bold text-gray-900 dark:text-slate-100">${rp.price.toFixed(2)}</span>
                      {rp.originalPrice && (
                        <span className="text-sm text-gray-400 dark:text-slate-500 line-through">${rp.originalPrice.toFixed(2)}</span>
                      )}
                    </div>
                  </div>
                </Link>
