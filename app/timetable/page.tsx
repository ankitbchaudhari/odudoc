...
export default function TimetablePage() {
  ...
  return (
    <>
      {/* Breadcrumb */}
      <div className="bg-white dark:bg-slate-900 border-b border-gray-100">
        <div className="mx-auto max-w-7xl px-4 py-3 sm:px-6 lg:px-8">
          <nav className="flex items-center gap-2 text-sm text-gray-500 dark:text-slate-400">
            <Link href="/" className="hover:text-primary-600 transition-colors">Home</Link>
            <span>/</span>
            <span className="text-gray-900 dark:text-slate-100 font-medium">Timetable</span>
          </nav>
        </div>
      </div>

      {/* Hero */}
      <section className="bg-gradient-to-br from-primary-50 via-white to-teal-50 dark:from-slate-900 dark:via-slate-950 dark:to-slate-900 py-12 md:py-16">
      ...
      {/* Filter */}
      <section className="border-b border-gray-100 bg-white dark:bg-slate-900 py-4">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-3">
              <label className="text-sm font-medium text-gray-700 dark:text-slate-300">Filter by Department:</label>
              <select
                value={filterDept}
                onChange={(e) => setFilterDept(e.target.value)}
                className="rounded-lg border border-gray-200 dark:border-slate-800 px-3 py-2 text-sm text-gray-600 dark:text-slate-300 outline-none focus:border-primary-500"
              >
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50 dark:bg-slate-900">
                  <th className="border-b border-r border-gray-200 dark:border-slate-800 px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-slate-400">
                    Time Slot
                  </th>
                  {DAYS.map((day) => (
                    <th
                      key={day}
                      className="border-b border-r border-gray-200 dark:border-slate-800 px-4 py-3 text-center text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-slate-400 last:border-r-0"
                    >
                      {day}
                    </th>
                  ))}
              ...
              <tbody>
                {TIME_SLOTS.map((slot) => (
                  <tr key={slot.key} className="border-b border-gray-100 last:border-b-0">
                    <td className="border-r border-gray-200 dark:border-slate-800 px-4 py-4 align-top">
                      <p className="text-sm font-semibold text-gray-900 dark:text-slate-100">{slot.label}</p>
                      <p className="text-xs text-gray-500 dark:text-slate-400">{slot.time}</p>
                    </td>
                    {DAYS.map((day) => {
                      ...
                      return (
                        <td
                          key={day}
                          className="border-r border-gray-200 dark:border-slate-800 px-2 py-2 align-top last:border-r-0"
                        >
                          <div className="space-y-2">
                            ...
                            {entries.length === 0 && (
                              <div className="rounded-lg bg-gray-50 dark:bg-slate-900 p-2.5 text-center text-xs text-gray-400 dark:text-slate-500">
                                --
                              </div>
                            )}
                          </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <div className="space-y-6">
            {DAYS.map((day) => {
              ...
              return (
                <div key={day} className="rounded-xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm overflow-hidden">
                      ...
                      return (
                        <div key={slot.key} className="p-4">
                          ...
                          {entries.length > 0 ? (
                            ...
                            <p className="text-sm text-gray-400 dark:text-slate-500">No appointments scheduled</p>
                          )}
                        </div>
                      );
                    })}
      ...
      {/* Legend */}
      <section className="border-t border-gray-100 bg-gray-50 dark:bg-slate-900 py-8">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h3 className="text-sm font-semibold text-gray-700 dark:text-slate-300 mb-4">Department Color Legend</h3>
      ...
      {/* CTA */}
      <section className="bg-white dark:bg-slate-900 py-12 border-t border-gray-100">
        <div className="mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
          <h2 className="text-xl font-bold text-gray-900 dark:text-slate-100">Ready to Book Your Appointment?</h2>
