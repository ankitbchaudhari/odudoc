    ...
    <main className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-emerald-50">
      <div className="mx-auto max-w-2xl px-4 py-12 sm:px-6">
        <header className="mb-6">
          ...
          <h1 className="mt-2 text-3xl font-extrabold text-slate-900 sm:text-4xl">Verify a medicine</h1>
    ...
    <section className={`mt-6 rounded-2xl border-2 p-6 ${tone}`}>
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-slate-600">{r.verdict === "verified" ? "Both checks cleared." : "See details below."}</p>
        </div>
      </div>

      {r.reasons.length > 0 && (
        <div className="mt-4">
          <p className="text-xs font-bold uppercase tracking-wider text-slate-700">Findings</p>
      ...
      {r.drug?.drug && (
        <div className="mt-4 rounded-xl bg-white/70 p-3 text-xs">
          <p className="font-bold text-slate-900">Brand match</p>
          <p className="mt-0.5 text-slate-700">
            <b>{r.drug.drug.brandName}</b>
            {r.drug.drug.genericName && <> · {r.drug.drug.genericName}</>}
            {r.drug.drug.strength && <> · {r.drug.drug.strength}</>}
          </p>
          {r.drug.drug.composition && <p className="text-slate-500">{r.drug.drug.composition}</p>}
          ...
          {r.drug.batch && (
            <p className="mt-1 text-[10px] text-slate-500">
              Batch <code className="rounded bg-slate-100 px-1">{r.drug.batch.batchNumber}</code>
      ...
      {r.partner?.partner && (
        <div className="mt-3 rounded-xl bg-white/70 p-3 text-xs">
          <p className="font-bold text-slate-900">Seller match</p>
          <p className="mt-0.5 text-slate-700">
            <b>{r.partner.partner.legalName}</b>
            {r.partner.partner.tradeName && <> · {r.partner.partner.tradeName}</>}
            <span className="ml-1 rounded-full bg-slate-100 px-1.5 py-0.5 text-[9px] uppercase tracking-wider">{r.partner.partner.kind}</span>
          </p>
          <p className="text-slate-500">{r.partner.partner.address}, {r.partner.partner.city}, {r.partner.partner.state}{r.partner.partner.pincode ? ` ${r.partner.partner.pincode}` : ""}</p>
