        ...
        <div className="rounded-xl border-2 border-rose-300 bg-rose-50 p-4 shadow-sm">
          ...
          <ul className="space-y-2">
            {result.redFlags.map((rf, i) => (
              <li key={i} className="rounded-lg bg-white p-3 ring-1 ring-rose-200">
                ...
                <p className="mt-1 text-xs text-slate-700"><strong>Action:</strong> {rf.action}</p>
              </li>
            ))}
          </ul>
        </div>
      ...
      {result.candidates.length > 0 && (
        <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
          <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50 px-4 py-2">
            <p className="text-sm font-bold text-slate-900">Differential ({result.candidates.length})</p>
            {loading && <span className="text-xs text-slate-500">analysing…</span>}
          </div>
                ...
                <li key={i} className="px-4 py-2.5">
                  <button onClick={() => {
                    ...
                    <div className="flex-1">
                      ...
                      <div className="mt-1 flex items-center gap-2">
                        <div className="h-1.5 flex-1 max-w-[200px] overflow-hidden rounded-full bg-slate-100">
                          <div className="h-full rounded-full bg-gradient-to-r from-emerald-500 to-emerald-400" style={{ width: `${pct}%` }} />
                        </div>
                        <span className="text-[10px] font-bold text-slate-500">{c.score}</span>
                      </div>
                    </div>
                  ...
                  {open && (
                    <div className="mt-2 space-y-1.5 border-l-2 border-indigo-200 pl-3 text-xs">
                      <p className="text-slate-700">{c.rationale}</p>
                      <p className="text-slate-700"><strong>Next step:</strong> {c.nextStep}</p>
