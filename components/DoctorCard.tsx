import Link from "next/link";
import type { Doctor } from "@/lib/data";

export default function DoctorCard({ doctor }: { doctor: Doctor }) {
  return (
    <div className="card group flex flex-col sm:flex-row sm:items-start sm:gap-5">
      {/* Avatar */}
      <div
        className={`mx-auto mb-4 flex h-20 w-20 flex-shrink-0 items-center justify-center rounded-full text-xl font-bold text-white sm:mx-0 sm:mb-0 ${doctor.imageColor}`}
      >
        {doctor.initials}
      </div>

      {/* Info */}
      <div className="flex-1 text-center sm:text-left">
        <Link href={`/doctors/${doctor.id}`} className="inline-block">
          <h3 className="text-lg font-semibold text-primary-600 transition-colors hover:text-primary-700">
            {doctor.name}
          </h3>
        </Link>
        <p className="text-sm text-gray-500">{doctor.specialty}</p>
        <p className="text-xs text-gray-400">{doctor.qualifications}</p>
        <div className="mt-2 flex flex-wrap items-center justify-center gap-3 text-sm text-gray-600 sm:justify-start">
          <span className="flex items-center gap-1">
            <svg className="h-4 w-4 text-yellow-500" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
            {doctor.rating}
            <span className="text-gray-400">({doctor.reviewCount})</span>
          </span>
          <span>{doctor.experience} yrs exp</span>
          <span>{doctor.city}</span>
        </div>
        <p className="mt-1 text-xs text-gray-400">{doctor.location}</p>
      </div>

      {/* Action */}
      <div className="mt-4 flex flex-col items-center gap-2 sm:mt-0">
        <p className="text-lg font-bold text-gray-900">${doctor.fee}</p>
        <Link
          href={`/doctors/${doctor.id}`}
          className="btn-primary !px-5 !py-2 !text-xs whitespace-nowrap"
        >
          Book Appointment
        </Link>
        <Link
          href="/consult"
          className="text-xs font-medium text-primary-600 hover:underline"
        >
          Video Consult
        </Link>
      </div>
    </div>
  );
}
