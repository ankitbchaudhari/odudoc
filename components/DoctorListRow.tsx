...
export default function DoctorListRow({ doctor }: { doctor: Doctor }) {
  ...
  return (
    <article className="group rounded-xl border border-gray-100 bg-white p-5 shadow-sm transition-all hover:border-primary-200 hover:shadow-md">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-start">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-sm text-gray-600">{doctor.specialty}</p>
              <p className="mt-1 text-sm text-gray-500">
                <span className="font-medium text-gray-700">{doctor.experience} years</span> experience overall
              </p>
              <p className="mt-1 text-xs text-gray-500 line-clamp-1">{doctor.qualifications}</p>
            </div>
          ...
          <div className="mt-3 flex items-start gap-1.5 text-sm text-gray-600">
            ...
            <div>
              ...
              {doctor.location && (
                <span className="text-gray-500"> · {doctor.location}</span>
              )}
            </div>
          </div>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-x-5 gap-y-2 text-sm">
            ...
            <div className="flex items-center gap-1 text-gray-500">
              ...
              <span className="text-gray-400">· {doctor.reviewCount} Patient Stories</span>
            </div>
        ...
        <div className="flex flex-row gap-2 sm:w-44 sm:flex-col">
          ...
          <div className="hidden text-center text-xs text-gray-400 sm:block">
            No Booking Fee
          </div>
        </div>
      </div>
