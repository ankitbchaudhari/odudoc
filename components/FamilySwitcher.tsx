      ...
      {open && (
        <div className="absolute right-0 z-40 mt-2 w-72 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl">
          ...
          <button
            ...
            className={`flex w-full items-center justify-between gap-2 px-4 py-3 text-left text-sm transition hover:bg-slate-50 ${active.kind === "self" ? "bg-indigo-50/40" : ""}`}
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-500">Primary account</p>
              </div>
            </div>
            ...
            {list.map((d) => {
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-center justify-between gap-2 border-t border-slate-50 px-4 py-2.5 text-left text-sm transition hover:bg-slate-50 ${isActive ? "bg-emerald-50/40" : ""}`}
                >
                  <div className="flex items-center gap-3">
                    <div className="flex h-9 w-9 items-center justify-center rounded-full bg-slate-100 text-base">{REL_EMOJI[d.relationship]}</div>
                    <div>
                      <p className="font-semibold text-slate-900">{d.name}</p>
                      <p className="text-[11px] text-slate-500 capitalize">{d.relationship.replace("_", " ")}</p>
                    </div>
                  </div>
          ...
          <Link href="/dashboard/family" onClick={() => setOpen(false)} className="block border-t border-slate-100 bg-slate-50 px-4 py-2.5 text-center text-xs font-semibold text-indigo-700 hover:bg-slate-100">
            + Manage family
          </Link>
        </div>
      )}
