    ...
    <div className="mx-auto max-w-2xl">
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-8 shadow-sm print:border-0 print:shadow-none">
        ...
        <div className="border-b-2 border-primary-600 pb-4">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-xs text-gray-500">Digital Health Platform</p>
            </div>
            <div className="text-right">
              <p className="text-sm font-semibold text-gray-900">Rx</p>
              <p className="text-xs text-gray-500">Prescription #{Date.now().toString(36).toUpperCase()}</p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-4 grid grid-cols-2 gap-4 border-b border-gray-100 pb-4">
          <div>
            <p className="text-xs font-medium uppercase text-gray-400">Prescribing Doctor</p>
            <p className="font-semibold text-gray-900">{doctorName}</p>
            <p className="text-sm text-gray-600">{doctorSpecialty}</p>
            <p className="text-xs text-gray-400">Reg. No: ODUDOC-{Math.random().toString(36).slice(2, 8).toUpperCase()}</p>
          </div>
          <div className="text-right">
            <p className="text-xs font-medium uppercase text-gray-400">Patient</p>
            <p className="font-semibold text-gray-900">{patientName}</p>
            <p className="text-sm text-gray-600">Date: {date}</p>
            <p className="text-xs text-gray-400">Via: Video Consultation</p>
          </div>
        </div>

        {/* Medications */}
        <div className="mt-6">
          <h3 className="mb-4 font-semibold text-gray-900">Medications</h3>
              ...
              <div key={i} className="rounded-lg bg-gray-50 p-4">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-gray-500">Note: {med.instructions}</p>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="mt-8 border-t border-gray-100 pt-4">
          <div className="flex items-end justify-between">
            <div>
              <p className="text-xs text-gray-400">This is a digitally generated prescription from a teleconsultation.</p>
      ...
      <div className="mt-6 flex justify-center gap-4 print:hidden">
        <button
          onClick={handlePrint}
          className="flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-6 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50"
        >
